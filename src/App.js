...
function App() {
  ...
  const formDataHandle = (key, value, boxId) => {
    let copyFormData = [...formData];
    if (key === "voteTitle") {
      copyFormData.map((v) => (v.voteTitle = value));
      setBoxTitle(value);
    } else if (key === "type") {
      copyFormData[boxId].type = value;
    } else if (key === "boxTitle") {
      copyFormData[boxId].boxTitle = value;
    } else if (key === "option") {
      copyFormData[boxId].option = value;
    } else if (key === "answerText") {
      copyFormData = copyFormData.map((v) => {
        if (v.boxId === boxId) {
          return { ...v, answerText: value };
        } else {
          return v;
        }
      });
      dispatch(updateForm(copyFormData));
    } else if (key === "boxChecked") {
      copyFormData = copyFormData.map((v) => {
        if (v.boxId === boxId) {
          return { ...v, boxChecked: value };
        } else {
          return v;
        }
      });
